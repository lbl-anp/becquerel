...
@pytest.fixture(
    params=[datetime.datetime.now(), datetime.date.today(), "2015-01-08 00:00:00", None]
)
def iq_date(request):
    return request.param
...
def test_isotopequantity_ref_atoms_rad(radioisotope, iq_kwargs):
    ...
    if "atoms" in iq_kwargs:
        ...
        assert isclose(
            iq.ref_atoms, iq_kwargs["uci"] * UCI_TO_BQ / radioisotope.decay_const
        )
...
@pytest.mark.parametrize(
    "date_in", ["2021-2-20 0:00:00", "2021-2-20", datetime.date(2021, 2, 20)]
)
...
def test_isotopequantity_from_decays(iq):
    ...
    # quantitative
    # if iq.half_life < 1000 * 3.156e7:  # floating point precision #65 # here
    n = iq.decays_from(start, stop)
    iq2 = IsotopeQuantity.from_decays(
        iq.isotope, n_decays=n, start_time=start, stop_time=stop
    )
    iq_at_start = float(iq.atoms_at(start))
    iq2_at_start = float(iq2.atoms_at(start))
    assert xp.isclose(
        iq_at_start, iq2_at_start, rtol=1e-8
    ), f"{100 * (1.0 - iq.atoms_at(start) / iq2.atoms_at(start)):.4f} %"
...
def test_isotopequantity_from_comparison(iq):
    ...
    iq2 = IsotopeQuantity.from_comparison(iq, counts1, interval1, counts1, interval1)
    assert iq2 == iq

    f = 3.1
    counts2 = counts1 * f
    iq3 = IsotopeQuantity.from_comparison(iq, counts1, interval1, counts2, interval1)
    assert iq3 == iq * f
    ...
    if iq.half_life < 1000 * 3.156e7:
        ...
        iq4 = IsotopeQuantity.from_comparison(
            iq, counts1, interval1, counts1, interval2
        )
        assert iq4 == iq * 2

        iq5 = IsotopeQuantity.from_comparison(
            iq, counts1, interval1, counts2, interval2
        )
        assert iq5 == iq * 2 * f

        interval3 = (start - dt, stop - dt)
        iq6 = IsotopeQuantity.from_comparison(
            iq, counts1, interval1, counts1, interval3
        )
        assert iq6 == iq / 2
...
@pytest.fixture(params=[("Cs-133", "Cs-134"), ("Hg-202", "Hg-203"), ("Na-23", "Na-24")])
