...
class SpcFile(SpectrumFile):
    ...
    def read(self, verbose=False):
        ...
        with open(self.filename, "rb") as f:
            ...
            if verbose:
                print(
                    "Done reading in SPC file.  Number of records: ", len(data_records)
                )
            ...
            for record_format in self.SPC_FORMAT_BEGINNING:
                if not (
                    ...
                    for j, data_format in enumerate(record_format):
                        ...
                        if verbose:
                            print(data_format[0], ": ", self.metadata[data_format[0]])
        ...
        self.metadata["Start date of sample collection"] = self.metadata[
        ...
        self.metadata["Stop date of sample collection"] = self.metadata[
            "Stop date of sample collection"
        ][:-1]
        if verbose:
            print(self.metadata["Stop date of sample collection"])
        ...
        self.location_description = self.metadata["Location Description Record 1"][3:]
        self.location_description = self.location_description.split("\x00\x00\x00")[
            0
        ].replace("\x00", "\n")
        if len(data_records) > 279:
            self.location_description += (
                self.metadata["Location Description Record 2"]
                .split("\x00\x00\x00")[0]
                .replace("\x00", "\n")
            )
