...
class CnfFile(SpectrumFile):
    ...
    def read(self, verbose=False):
        ...
        for i in range(112, 128 * 1024, 48):
            ...
            if (
                (data[i + 1] == 0x20 and data[i + 2] == 0x01)
                or (data[i + 1] == 0)
                or (data[i + 2] == 0)
            ):
                if data[i] == 0:
        ...
        if (
            (offset_sam + 48 + 80) >= len(data)
            or data[offset_sam] != 1
        ...
        if (
            (offset_acq + 48 + 128 + 10 + 4) >= len(data)
        ...
        if (
            offset_chan + 512 + 4 * self.num_channels > len(data)
            or data[offset_chan] != 5
            or data[offset_chan + 1] != 0x20
        ):
