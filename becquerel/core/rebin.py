...
def _check_ndim(arr, ndim, arr_name="array"):
    ...
    if (arr.ndim != ndim) and (arr.ndim not in ndim):
        raise RebinError("{}({}) is not {}D".format(arr_name, arr.shape, ndim))
...
def _check_shape(arr0, arr1, arr0_name="array0", arr1_name="array1", edges=False):
...
def _rebin2d_listmode(in_spectra, in_edges, out_edges, slopes):
    ...
    """
    # Init output
    out_spectra = np.zeros((in_spectra.shape[0], out_edges.shape[0] - 1), np.int64)
    for i in np.arange(in_spectra.shape[0]):
        out_spectra[i, :] = _rebin_listmode(
            in_spectra[i, :], in_edges[i, :], out_edges, slopes[i, :]
        )
    return out_spectra
...
def rebin(
    ...
    if method == "interpolation":
        if in_spectra.ndim == 2:
            return _rebin2d_interpolation(in_spectra, in_edges, out_edges, slopes)
